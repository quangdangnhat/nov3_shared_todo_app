  ...
  group('TreeBuilderService Tests', () {
    ...
    group('buildTreeFromLists', () {
      ...
      test('should add TodoList nodes to root', () async {
        ...
        expect((result.childrenAsList.first as TreeNode<TreeNodeData>).data?.id,
            'list-123');
        expect(
            (result.childrenAsList.first as TreeNode<TreeNodeData>).data?.name,
            'Test List');
      });
    ...
    group('buildFolderNode', () {
      ...
      test('should add tasks to folder node', () async {
        ...
        expect(
            (taskNodes.first as TreeNode<TreeNodeData>).data?.id, 'task-789');
      });
      ...
      test('should add sub folders recursively', () async {
        ...
        expect((folderNodes.first as TreeNode<TreeNodeData>).data?.id,
            'sub-folder-456');
      });
    ...
    group('Complex Tree Structures', () {
      test('should build deep nested tree', () async {
        ...
        expect(result.childrenAsList.length, 1);
        expect(
            (result.childrenAsList.first as TreeNode<TreeNodeData>)
                .childrenAsList
                .length,
            1);
        expect(
            ((result.childrenAsList.first as TreeNode<TreeNodeData>)
                    .childrenAsList
                    .first as TreeNode<TreeNodeData>)
                .data
                ?.id,
            'folder-level-2');
      });
      ...
      test('should handle multiple tasks in folder', () async {
        ...
        expect(
            result.childrenAsList.every((node) =>
                (node as TreeNode<TreeNodeData>).data?.type == NodeType.task),
            isTrue);
      });
